<div align="center">

<!-- TYPING ANIMATION -->
<a href="https://git.io/typing-svg">
  <img src="https://readme-typing-svg.demolab.com?font=Orbitron&weight=700&size=22&pause=1000&color=E8002D&center=true&vCenter=true&width=700&lines=B.S.+Electrical+%26+Computer+Engineering+%E2%80%94+NJIT+%E2%80%9826;Motorsport+Engineer+in+the+Making+%F0%9F%8F%8E%EF%B8%8F;Built+to+Race.+Engineered+to+Win.;From+Lightning+McQueen+to+Formula+1." alt="Typing SVG" />
</a>

<br/><br/>

<!-- MOTTO BADGE -->
> *"The mechanics of success are identical to the mechanics of a racing car — everything must work together, nothing is excess, and precision is everything."*

<br/>

[![LinkedIn](https://img.shields.io/badge/LinkedIn-Marco%20Antonio-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white)](https://www.linkedin.com/in/txniiii/)
[![Email](https://img.shields.io/badge/Email-Get%20In%20Touch-E8002D?style=for-the-badge&logo=gmail&logoColor=white)](mailto:mabautista358@gmail.com)
[![GitHub](https://img.shields.io/badge/GitHub-Follow-1E1E1E?style=for-the-badge&logo=github&logoColor=white)](https://github.com)

</div>

---

## 🏁 About Me

Hey! I'm **Marco Antonio Bautista**, a freshly minted **B.S. Electrical & Computer Engineer** from the **New Jersey Institute of Technology (NJIT)**, class of **May 22nd, 2026**.

I'm not just someone who loves fast cars. I'm someone who grew up *dreaming* in lap times, who learned to love engineering through the lens of motorsport, and who has spent years building the technical foundation to make that dream a professional reality.

My goal is simple and unwavering: **to work in Formula 1.**

I want to contribute to a team that pushes the limits of what machines and humans can do. I don't need the perfect seat. I need an opportunity to learn, grow, and give everything I have.

---

## 🛣️ My Journey

```
📍 New Brunswick High School  →  High School Diploma
         ↓
📍 New Jersey Institute of Technology (NJIT)
         ↓
🎓 B.S. Electrical & Computer Engineering — May 22, 2026
         ↓
🏎️  Formula 1 — The destination I've been engineering toward my entire life.
```

From the hallways of New Brunswick to the labs of NJIT, every circuit I designed, every simulation I ran, every line of embedded code I wrote — it was all pointing in one direction.

---

## 🏎️ Why Formula 1?

It started when I was **4 years old**.

*Cars* (2006). Lightning McQueen. The roar of engines, the blur of speed, the drama of competition. To most kids, it was a movie. To me, it was a **calling**.

That spark never faded. It grew into a passion for real racing, for the engineering stories behind every tenth of a second. I started watching **Formula 1** and became obsessed — not just with who won, but *how*. The aerodynamics, the telemetry, the tire strategies, the relentless iteration.

Formula 1 is the **pinnacle of applied engineering**. Every component is optimized to the atom. Every millisecond is fought for with data. It's a sport where the best engineers in the world compete through machines, and there is nowhere else I want to be.

I've been building toward this my entire life — now it's time to show what I can do.

---

## 🔬 Projects

### 🏎️ Motorsport Vehicle Telemetry Logger
> *Real-time data acquisition for performance analysis — the kind of system that wins races.*

Designed and built a **vehicle telemetry data logger** capable of capturing real-time sensor data from a moving vehicle. The system integrates embedded hardware with custom firmware to sample, encode, and log critical performance metrics — speed, acceleration, temperature, and more. Data is structured for post-session analysis, mimicking the pipeline used in professional motorsport environments. Built with a performance-first mindset: low latency, high reliability, and clean data output.

**Stack:** C/C++, Embedded Systems, Data Acquisition, Sensors, PCB Design

---

### 🤖 Smart-Auger Multi-Pet Identification & Dispensing System
> *Precision control meets intelligent sensing — because every system deserves a smart brain.*

Engineered an intelligent automated pet feeder that uses **multi-pet identification** to dispense the correct food portion to the correct animal. The system combines embedded microcontroller logic, sensor fusion, and mechanical actuation through an auger dispensing mechanism. The result: a closed-loop, autonomous feeding system with real-world reliability constraints — a mirror of how control systems operate in automotive and industrial applications.

**Stack:** Embedded Systems, C/C++, Sensors, Industrial Controls, PCB Design

---

### 🌐 F1 Planner Web App
> *Strategy is as important as speed — so I built a tool to plan both.*

Developed a **Formula 1-themed web application** that allows users to plan, visualize, and interact with F1 season data. The app reflects my deep knowledge of the sport and my ability to translate passion into functional, user-facing software. Designed with clean UI/UX principles, dynamic data rendering, and a motorsport aesthetic that makes it feel native to the paddock.

**Stack:** JavaScript, HTML/CSS, PHP, SQL, Web Development

---

## ⚙️ Technical Skills

<div align="center">

### 💻 Programming & Software
![Python](https://img.shields.io/badge/Python-3776AB?style=flat-square&logo=python&logoColor=white)
![C/C++](https://img.shields.io/badge/C%2FC%2B%2B-00599C?style=flat-square&logo=c%2B%2B&logoColor=white)
![MATLAB](https://img.shields.io/badge/MATLAB-0076A8?style=flat-square&logo=mathworks&logoColor=white)
![JavaScript](https://img.shields.io/badge/JavaScript-F7DF1E?style=flat-square&logo=javascript&logoColor=black)
![SQL](https://img.shields.io/badge/SQL-4479A1?style=flat-square&logo=mysql&logoColor=white)
![PHP](https://img.shields.io/badge/PHP-777BB4?style=flat-square&logo=php&logoColor=white)
![HTML/CSS](https://img.shields.io/badge/HTML%2FCSS-E34F26?style=flat-square&logo=html5&logoColor=white)
![Linux](https://img.shields.io/badge/Linux-FCC624?style=flat-square&logo=linux&logoColor=black)
![Git](https://img.shields.io/badge/Git%2FGitHub-181717?style=flat-square&logo=github&logoColor=white)
![Fortran](https://img.shields.io/badge/Fortran-734F96?style=flat-square&logo=fortran&logoColor=white)

### 🔌 Electrical & Computer Engineering
`Embedded Systems` │ `Data Acquisition` │ `PCB Design` | `Instrumentation` │ `Industrial Controls`│ `Signal Processing`

### 🖥️ CAD & Simulation
![Fusion 360](https://img.shields.io/badge/Fusion%20360-FF6D00?style=flat-square&logo=autodesk&logoColor=white)
![SolidWorks](https://img.shields.io/badge/SolidWorks-CC0000?style=flat-square&logo=dassaultsystemes&logoColor=white)
![CATIA](https://img.shields.io/badge/CATIA-005BAC?style=flat-square&logo=dassaultsystemes&logoColor=white)
![AutoCAD](https://img.shields.io/badge/AutoCAD-CA2424?style=flat-square&logo=autodesk&logoColor=white)
![ANSYS](https://img.shields.io/badge/ANSYS-FFB71B?style=flat-square&logo=ansys&logoColor=black)
![Simulink](https://img.shields.io/badge/Simulink-0076A8?style=flat-square&logo=mathworks&logoColor=white)

`NUMECA` · `CONCEPT Tools` · `Flex5/HAWC2`

### 🎨 Design & Productivity
![Illustrator](https://img.shields.io/badge/Adobe%20Illustrator-FF9A00?style=flat-square&logo=adobeillustrator&logoColor=white)
![CorelDRAW](https://img.shields.io/badge/CorelDRAW-00A550?style=flat-square&logoColor=white)
![Microsoft Office](https://img.shields.io/badge/Microsoft%20Office-D83B01?style=flat-square&logo=microsoftoffice&logoColor=white)

</div>

---

## 📜 Certifications

| 🏅 Certification | Issuing Body |
|---|---|
| *(Add your certifications here — e.g. CompTIA, MathWorks, etc.)* | *(Issuer)* |

---

## 🚀 What I'm Working Toward

The checkered flag isn't the end — it's the beginning of the next lap.

Right now, I'm focused on:

- 🏗️ **Deepening my telemetry & data systems expertise** — building tools that mirror what F1 teams use trackside
- 📡 **Advancing my embedded systems knowledge** — reliability, low-latency comms, and real-time control
- 📊 **Learning more about vehicle dynamics & CFD** — understanding the physics that makes an F1 car fast
- 🌍 **Connecting with the motorsport community** — engineers, teams, anyone who shares this passion

I'm not waiting for permission to pursue this dream. I'm building toward it every single day.

---

## 🤝 Let's Connect

<div align="center">

Whether you're a recruiter at an F1 team, an engineer in motorsport, or someone who just loves racing — **I want to hear from you.**

I'm actively seeking opportunities to break into motorsport engineering. If your team needs someone who is technically sharp, endlessly passionate, and willing to outwork anyone in the room — let's talk.

<br/>

[![LinkedIn](https://img.shields.io/badge/Connect%20on%20LinkedIn-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white)](https://linkedin.com)
[![Email](https://img.shields.io/badge/Send%20Me%20an%20Email-E8002D?style=for-the-badge&logo=gmail&logoColor=white)](mailto:marcobautista@email.com)

<br/>

---

*"To finish first, first you must finish. But to make it to the grid — you have to dare to start."*

**— Marco Antonio Bautista, NJIT ECE '26 🏁**

<br/>

<img src="https://capsule-render.vercel.app/api?type=waving&color=E8002D,FF0000,1E1E1E&height=120&section=footer" width="100%"/>

</div>
